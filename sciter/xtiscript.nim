...
type
  tiscript_get_prop* = proc (c: ptr tiscript_VM;
...
type
  tiscript_on_gc_copy* = proc (instance_data: pointer;
                              new_self: tiscript_value) {.cdecl.}
...
type
  ...
  tiscript_native_interface* = object
    ...
    create_vm*: proc (features: cuint; ## #= 0xffffffff   
                    heap_size: cuint; ## #= 1*1024*1024
                    stack_size: cuint): ptr tiscript_VM {.cdecl.} ## #= 64*1024
    ## # destroy tiscript_VM
    destroy_vm*: proc (pvm: ptr tiscript_VM) {.cdecl.} 
    ## # invoke GC
    invoke_gc*: proc (pvm: ptr tiscript_VM) {.cdecl.}
    ## # set stdin, stdout and stderr for this tiscript_VM
    set_std_streams*: proc (pvm: ptr tiscript_VM; input: ptr tiscript_stream;
                          output: ptr tiscript_stream;
                          error: ptr tiscript_stream) {.cdecl.}
    ## # get tiscript_VM attached to the current thread
    get_current_vm*: proc (): ptr tiscript_VM {.cdecl.}
    ## # get global namespace (Object)
    get_global_ns*: proc (a2: ptr tiscript_VM): tiscript_value {.cdecl.} 
    ...
    get_datetime*: proc (a2: ptr tiscript_VM;
                        v: tiscript_value; dt: ptr culonglong): bool {.
    ...
    ## define native class
    define_class*: proc (vm: ptr tiscript_VM; ## # in this tiscript_VM
                        cls: ptr tiscript_class_def; 
                        zns: tiscript_value): tiscript_value {.cdecl.} ## # in this namespace object (or 0 if global)      
    ## # object
    ...
    set_instance_data*: proc (obj: tiscript_value; data: pointer) {.cdecl.} 
    ## # array
    ...
    set_elem*: proc (a2: ptr tiscript_VM; obj: tiscript_value; idx: cuint;
                   tiscript_value: tiscript_value): bool {.cdecl.}
    get_elem*: proc (a2: ptr tiscript_VM;
                    obj: tiscript_value; idx: cuint): tiscript_value {.
        cdecl.}
    set_array_size*: proc (a2: ptr tiscript_VM;
                          obj: tiscript_value;
                          of_size: cuint): tiscript_value {.
        cdecl.}
    get_array_size*: proc (a2: ptr tiscript_VM;
                          obj: tiscript_value): cuint {.cdecl.}
    
    ## eval
    eval*: proc (a2: ptr tiscript_VM;
                ns: tiscript_value;
                input: ptr tiscript_stream;
                template_mode: bool;
                pretval: ptr tiscript_value): bool {.cdecl.}
    eval_string*: proc (a2: ptr tiscript_VM; ns: tiscript_value;
                      script: ptr Utf16Char; script_length: cuint;
                      pretval: ptr tiscript_value): bool {.cdecl.}
        
    ## call function (method)
    call*: proc (a2: ptr tiscript_VM; obj: tiscript_value;
                function: tiscript_value;
                argv: ptr tiscript_value; argn: cuint;
                pretval: ptr tiscript_value): bool {.cdecl.}

    ## # compiled bytecodes
    compile*: proc (pvm: ptr tiscript_VM; input: ptr tiscript_stream;
                  output_bytecodes: ptr tiscript_stream;
                  template_mode: bool): bool {.cdecl.}
    loadbc*: proc (pvm: ptr tiscript_VM;
                  input_bytecodes: ptr tiscript_stream): bool {.cdecl.}

    ## # throw error
    throw_error*: proc (a2: ptr tiscript_VM; error: ptr Utf16Char) {.cdecl.}

    ## # arguments access
    get_arg_count*: proc (pvm: ptr tiscript_VM): cuint {.cdecl.}
    get_arg_n*: proc (pvm: ptr tiscript_VM; n: cuint): tiscript_value {.cdecl.}

    ## # path here is global "path" of the object, something like
    ## # "one"
    ## # "one.two", etc.
    get_value_by_path*: proc (pvm: ptr tiscript_VM;
                              v: ptr tiscript_value; path: cstring): bool {.cdecl.}
    
    ## # pins
    pin*: proc (a2: ptr tiscript_VM; pp: ptr tiscript_pvalue) {.cdecl.}
    unpin*: proc (pp: ptr tiscript_pvalue) {.cdecl.} 
    ...
    post*: proc (pvm: ptr tiscript_VM; pfunc: ptr tiscript_callback;
                 prm: pointer): bool {.cdecl.}
    ...
    set_remote_std_streams*: proc (pvm: ptr tiscript_VM; input: ptr tiscript_pvalue;
                                 ...
                                 error: ptr tiscript_pvalue): bool {.cdecl.} 
    ## # support of multi-return values from native fucntions, n here is a number 1..64    
    make_val_list*: proc (pvm: ptr tiscript_VM;
                    valc: cint; va: ptr tiscript_value): tiscript_value {.cdecl.}
    ## # returns number of props in object, elements in array, or bytes in byte array.
    get_length*: proc (pvm: ptr tiscript_VM; obj: tiscript_value): cint {.cdecl.} 
    ## # for( var val in coll ) {...}
    ...
    get_next_key_value*: proc (pvm: ptr tiscript_VM; obj: ptr tiscript_value;
                             ...
                             val: ptr tiscript_value): bool {.cdecl.}
