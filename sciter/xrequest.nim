...
type
  ...
  SciterRequestAPI* = object
    ## # a.k.a AddRef()
    RequestUse*: proc (rq: HREQUEST): REQUEST_RESULT {.stdcall.} 
    ## # a.k.a Release()
    RequestUnUse*: proc (rq: HREQUEST): REQUEST_RESULT {.stdcall.} 
    ## # get requested URL
    RequestUrl*: proc (rq: HREQUEST; rcv: ptr LPCSTR_RECEIVER; 
    ...
    RequestGetRequestType*: proc (rq: HREQUEST; 
                                  pType: ptr REQUEST_RQ_TYPE): REQUEST_RESULT {.stdcall.}
    ## # get requested data type
    ...
    RequestGetNumberOfParameters*: proc (rq: HREQUEST;
    ...
    RequestGetNumberOfRqHeaders*: proc (rq: HREQUEST;
                                        pNumber: ptr uint32): REQUEST_RESULT {.stdcall.}               
    ...
    RequestGetProxyHost*: proc (rq: HREQUEST; rcv: ptr LPCSTR_RECEIVER;
                              rcv_param: pointer): REQUEST_RESULT {.stdcall.} 
    ## # get proxy port
    RequestGetProxyPort*: proc (rq: HREQUEST;
                               pPort: ptr uint32): REQUEST_RESULT {.stdcall.} 
    ##  mark  reequest  as  complete  with  status  and  data 
    RequestSetSucceeded*: proc (rq: HREQUEST;
                                status: uint32; dataOrNull: pointer;
    ...
    RequestAppendDataChunk*: proc (rq: HREQUEST; data: pointer;
                                   dataLength: uint32): REQUEST_RESULT {.stdcall.}
