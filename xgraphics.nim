...
type
  ...
  image_write_function* = proc (prm: pointer; data: ptr byte; data_length: uint32): bool {.stdcall.}
  SciterGraphicsAPI* = object
    imageCreate*: proc (poutImg: ptr HIMG; width: uint32; height: uint32; withAlpha: bool): GRAPHIN_RESULT {.stdcall.}               
    ## # image primitives
    ...
    imageLoad*: proc (bytes: ptr byte; num_bytes: uint32; pout_img: ptr HIMG): GRAPHIN_RESULT {.stdcall.}  
    ## # png: 0, jpeg:, 10 - 100 
    imageSave*: proc (himg: HIMG; pfn: ptr image_write_function; prm: pointer; 
                bpp: uint32; quality: uint32): GRAPHIN_RESULT {.stdcall.} 
    ## # 
    ## SECTION: 
    ## graphics primitives and drawing operations create 
    ## COLOR value
    ## # save png/jpeg/etc. image to stream of bytes
    ## # function and its param passed "as is" 
    ## #24,32 if alpha needed                    
    RGBA*: proc (red: uint32; green: uint32; blue: uint32; alpha: uint32): COLOR {.stdcall.} ## #= 255
    ...
    gLine*: proc (hgfx: HGFX; x1: POS; y1: POS; x2: POS; y2: POS): GRAPHIN_RESULT {.stdcall.} 
    ## # 
    ##   Draws rectangle using current lineColor/lineGradient and fillColor/fillGradient 
    ## # with (optional) rounded corners.
    gRectangle*: proc (hgfx: HGFX; x1: POS; y1: POS; x2: POS; y2: POS): GRAPHIN_RESULT {.stdcall.}               
    ## # Draws rounded rectangle using current lineColor/lineGradient and 
    ## # fillColor/fillGradient with (optional) rounded corners.
    gRoundedRectangle*: proc (hgfx: HGFX; x1: POS; y1: POS; x2: POS; y2: POS; radii8: ptr DIM): GRAPHIN_RESULT {.stdcall.}
    ## # Draws circle or ellipse using current lineColor/lineGradient and fillColor/fillGradient.
    ## # DIM[8] - four rx/ry pairs 
    gEllipse*: proc (hgfx: HGFX; x: POS; y: POS; rx: DIM; ry: DIM): GRAPHIN_RESULT {.stdcall.} 
    ## # Draws closed arc using current lineColor/lineGradient and fillColor/fillGradient.
    gArc*: proc (hgfx: HGFX; x: POS; y: POS; rx: POS; ry: POS; start: ANGLE; sweep: ANGLE): GRAPHIN_RESULT {.stdcall.}
    ## # Draws star.
    gStar*: proc (hgfx: HGFX; x: POS; y: POS; r1: DIM; r2: DIM; start: ANGLE; rays: uint32): GRAPHIN_RESULT {.stdcall.}
    ## # Closed polygon.
    gPolygon*: proc (hgfx: HGFX; xy: ptr POS; num_points: uint32): GRAPHIN_RESULT {.stdcall.} 
    ## Polyline.
    gPolyline*: proc (hgfx: HGFX; xy: ptr POS; num_points: uint32): GRAPHIN_RESULT {.stdcall.}     
    ## SECTION: 
    ## Path operations
    ...
    pathMoveTo*: proc (path: HPATH; x: POS; y: POS; relative: bool): GRAPHIN_RESULT {.stdcall.}
    pathLineTo*: proc (path: HPATH; x: POS; y: POS; relative: bool): GRAPHIN_RESULT {.stdcall.}
    pathArcTo*: proc (path: HPATH; x: POS; y: POS; angle: ANGLE; rx: DIM; ry: DIM;
                    is_large_arc: bool; clockwise: bool; relative: bool): GRAPHIN_RESULT {.stdcall.}
    ...
    gDrawPath*: proc (hgfx: HGFX; path: HPATH; dpm: DRAW_PATH_MODE): GRAPHIN_RESULT {.stdcall.}               
    ...
    gSkew*: proc (hgfx: HGFX; dx: DIM; dy: DIM): GRAPHIN_RESULT {.stdcall.} ## # all above in one shot
    gTransform*: proc (hgfx: HGFX; m11: POS; m12: POS; m21: POS; m22: POS; dx: POS; dy: POS): GRAPHIN_RESULT {.stdcall.}
    ...
    gLineJoin*: proc (hgfx: HGFX; `type`: SCITER_LINE_JOIN_TYPE): GRAPHIN_RESULT {.stdcall.}
    gLineCap*: proc (hgfx: HGFX; `type`: SCITER_LINE_CAP_TYPE): GRAPHIN_RESULT {.stdcall.} 
    ## #GRAPHIN_RESULT SCFN
    ## #      (*gNoLine ( HGFX hgfx ) { gLineWidth(hgfx,0.0); }
    ## # COLOR for solid lines/strokes
    gLineColor*: proc (hgfx: HGFX; c: COLOR): GRAPHIN_RESULT {.stdcall.} ## # COLOR for solid fills
    ...
    gLineGradientLinear*: proc (hgfx: HGFX; x1: POS; y1: POS; x2: POS; y2: POS;
                              stops: ptr COLOR_STOP; nstops: uint32): GRAPHIN_RESULT {.stdcall.}
    ...
    textCreateForElement*: proc (ptext: ptr HTEXT; text: WideCString;
                               textLength: uint32; he: HELEMENT): GRAPHIN_RESULT {.stdcall.}               
    ...
    textSetBox*: proc (text: HTEXT; width: DIM; height: DIM): GRAPHIN_RESULT {.stdcall.}
    ## # 
    ## draw  text  with  position  (1..9 on  MUMPAD) at  px,py 
    ## Ex: 
    ## gDrawText(  100,100,5) 
    ## will  draw  text box  with its  center at  100,100 px
    gDrawText*: proc (hgfx: HGFX; text: HTEXT; px: POS; py: POS; position: uint32): GRAPHIN_RESULT {.stdcall.}               
    ## # SECTION: image rendering
    ## # draws img onto the graphics surface with current transformation applied (scale, rotation).
    gDrawImage*: proc (hgfx: HGFX; himg: HIMG; x: POS; y: POS; w: ptr DIM;     
        h: ptr DIM; ## #= 0
        ix: ptr uint32; ## #= 0
        iy: ptr uint32; ## #= 0
        iw: ptr uint32; ## #= 0
        ih: ptr uint32; opacity: ptr cfloat): GRAPHIN_RESULT {.stdcall.} ## #= 0, if provided is in 0.0 .. 1.0
    ## # SECTION: coordinate space
    gWorldToScreen*: proc (hgfx: HGFX; inout_x: ptr POS; inout_y: ptr POS): GRAPHIN_RESULT {.stdcall.} 
    ## #inline GRAPHIN_RESULT
    ## # graphics_world_to_screen ( HGFX hgfx, POS* length)
    ## # {
    ## #   return graphics_world_to_screen ( hgfx, length, 0);
    ## # }
    gScreenToWorld*: proc (hgfx: HGFX; inout_x: ptr POS; inout_y: ptr POS): GRAPHIN_RESULT {.stdcall.}
    ## # inline GRAPHIN_RESULT
    ...
    ## # }
    ## # SECTION: clipping
    gPushClipBox*: proc (hgfx: HGFX; x1: POS; y1: POS; x2: POS; y2: POS; opacity: cfloat): GRAPHIN_RESULT {.stdcall.}
    ## #=1.f
    gPushClipPath*: proc (hgfx: HGFX; hpath: HPATH; opacity: cfloat): GRAPHIN_RESULT {.stdcall.}               
